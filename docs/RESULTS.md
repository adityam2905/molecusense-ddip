# MolecuSense: detailed results

The full evidence behind the short summary in the [README](../README.md).
Every number comes from a JSON file in [`results/`](../results/).

**Contents:** [Results](#results) · [How it's built](#how-its-built) ·
[Tests](#tests) · [Limitations](#limitations) · [Bugs found and fixed](#bugs-found-and-fixed)
...
### RL experiment (removed)

A reinforcement-learning layer that nudged the probability was trained on
validation pairs of the 10k model and tested once: **62.0% → 62.2% accuracy**
(2 of 998 pairs), which is noise. It was removed from the app and then from the
code; the result is kept in [`results/rl_5k.json`](../results/rl_5k.json) and
the code is in the git history.
